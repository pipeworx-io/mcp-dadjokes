...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Dad Jokes');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'random_joke',
    description: 'Fetch one random dad joke from icanhazdadjoke.com. Returns joke text and alphanumeric ID.',
    inputSchema: {
      type: 'object',
      properties: {},
    },
  ...
  {
    name: 'search_jokes',
    description: 'Search dad jokes by keyword or topic. Returns matching jokes with IDs and text.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
  ...
  {
    name: 'get_joke',
    description: 'Retrieve a specific dad joke by ID. Returns the full joke text.',
    inputSchema: {
      type: 'object',
      properties: {
        id: {
...
async function randomJoke() {
  const res = await pwFetch(`${BASE_URL}/`, { headers: HEADERS });
  if (!res.ok) throw await httpError(res, 'icanhazdadjoke error');
...
async function getJoke(id: string) {
  const res = await pwFetch(`${BASE_URL}/j/${encodeURIComponent(id)}`, { headers: HEADERS });
  if (!res.ok) throw await httpError(res, 'icanhazdadjoke error');
  const data = (await res.json()) as RawJoke;
  return formatJoke(data);
}
